...
from ..models import Job, User, Company,Group,Member,Event
...
def index():
    ...
    groups = Group.query.limit(8)
    members = Member.query.limit(8)
    events = Event.query.limit(6)
    return render_template('index.html', active='index', members=members,jobs=jobs, events=events,groups=groups,companies=companies)
...
def search():
    ...
    if search_type == 'event':
        return redirect(url_for('event.index', kw=kw))
    elif search_type == 'group':
        return redirect(url_for('group.index', kw=kw))
    else:
        return redirect(url_for('member.index', kw=kw))
