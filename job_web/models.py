...
class Group(Base1):
    ...
    localized_country_name = db.Column(db.String(256))
    members = db.Column(db.Integer)
    events_id_list = db.Column(db.String(256))
    country = db.Column(db.String(256))
    organizer_id = db.Column(db.Integer)
    organizer_name = db.Column(db.String(256))
    category_id = db.Column(db.Integer)
    category_name = db.Column(db.String(256))
    meta_category_id = db.Column(db.Integer)
    meta_category_name = db.Column(db.String(256))
    is_enable = db.Column(db.Boolean, default=True, index=True)
    @property
    def events_id_list_all(self):
        if self.events_id_list and '，' in self.events_id_list:
            return self.events_id_list.split('，')
        return self.events_id_list.split(',')

    def enabled_events(self):
        return self.events.filter(Event.is_enable.is_(True))
...
class Event(Base1):
    ...
    created = db.Column(db.DateTime)
    status= db.Column(db.String(256))
    local_date= db.Column(db.DateTime)
    updated= db.Column(db.DateTime)
    group_id= db.Column(db.Integer)
    group_created= db.Column(db.DateTime)
    link= db.Column(db.String(256))
    attendance_count= db.Column(db.Integer)
    name= db.Column(db.String(256))
    group_name=db.Column(db.String(256))
    attendees_id_list= db.Column(db.String(1000))
    is_enable = db.Column(db.Boolean, default=True, index=True)
    group_id = db.Column(db.Integer, db.ForeignKey('group.id', ondelete='CASCADE'))
    group = db.relationship('Group', uselist=False, backref=db.backref('events', lazy='dynamic'))
    @property
    def attendees_id_list(self):
        if self.attendees_id_list and '，' in self.attendees_id_list:
            return self.attendees_id_list.split('，')
        return self.attendees_id_list.split(',')

    def enabled_event(self):
        return self.event.filter(Event.is_enable.is_(True))
...
class Member(Base1):
    ...
    is_pro_admin= db.Column(db.String(3))

    is_enable = db.Column(db.Boolean, default=True, index=True)
    @property

    def enabled_member(self):
        return self.member.filter(Member.is_enable.is_(True))
